...
class MainFragment : Fragment() {
    ...
    private fun onButtonClick(position : Int) = when(position) {
        0 -> router.navigateTo { ListFragment.createListFragment(TABLE) }
        1 -> router.navigateTo { ListFragment.createListFragment(TABLE) }
        2 -> router.navigateTo { ListFragment.createListFragment(TABLE) }
